...
toggle_node() {
    ...
    # 确保 disabled_inbounds 数组存在，如果不存在则创建
    if ! jq -e '.disabled_inbounds' ${CONFIG_FILE} > /dev/null; then
        jq '. + {"disabled_inbounds": []}' ${CONFIG_FILE} > ${CONFIG_FILE}.tmp && mv ${CONFIG_FILE}.tmp ${CONFIG_FILE}
    fi

    echo "请选择操作："
    echo "1. 禁用活动节点"
    echo "2. 启用禁用节点"
    read -p "请选择 [1-2]: " action_choice

    # 备份配置
    cp ${CONFIG_FILE} ${CONFIG_FILE}.bak_toggle_$(date +%Y%m%d%H%M%S)
    echo -e "${YELLOW}已创建配置文件备份: ${CONFIG_FILE}.bak_toggle_$(date +%Y%m%d%H%M%S)${PLAIN}"

    local temp_config=$(cat ${CONFIG_FILE}) # 读取当前配置用于操作

    if [[ "$action_choice" == "1" ]]; then
        # --- 禁用活动节点 ---
        echo -e "${YELLOW}当前活动的节点列表:${PLAIN}"
        declare -a active_node_indices
        declare -a active_node_descriptions
        declare -a active_is_reality
        declare -a active_reality_vless_index
        declare -a active_reality_dokodemo_tag

        inbound_count=$(jq '.inbounds | length' <<< "$temp_config")
        count=0
        processed_vless_indices=() # 存储已作为REALITY一部分处理的vless索引

        for ((i=0; i<${inbound_count}; i++)); do
            # 检查此索引是否已被作为REALITY的vless部分处理过
            skip=false
            for processed_idx in "${processed_vless_indices[@]}"; do
                if [[ "$i" == "$processed_idx" ]]; then
                    skip=true
                    break
                fi
            done
            ...
            protocol=$(jq -r ".inbounds[$i].protocol" <<< "$temp_config")
            port=$(jq -r ".inbounds[$i].port" <<< "$temp_config")
            tag=$(jq -r ".inbounds[$i].tag // \"未命名\"" <<< "$temp_config")
            listen=$(jq -r ".inbounds[$i].listen // \"0.0.0.0\"" <<< "$temp_config")

            is_reality_pair=false
            vless_idx=-1
            dokodemo_tag=""

            # 检查是否是REALITY的dokodemo-door部分
            if [[ "$protocol" == "dokodemo-door" && ("$tag" == "dokodemo-in" || "$tag" =~ ^dokodemo-in-[0-9]+$) ]]; then
                internal_port=$(jq -r ".inbounds[$i].settings.port" <<< "$temp_config")
                # 寻找对应的vless
                for ((j=0; j<${inbound_count}; j++)); do
                    if [[ $i -ne $j ]]; then
                        inner_protocol=$(jq -r ".inbounds[$j].protocol" <<< "$temp_config")
                        inner_port=$(jq -r ".inbounds[$j].port" <<< "$temp_config")
                        inner_listen=$(jq -r ".inbounds[$j].listen // \"0.0.0.0\"" <<< "$temp_config")
                        security=$(jq -r ".inbounds[$j].streamSettings.security // \"none\"" <<< "$temp_config")

                        if [[ "$inner_protocol" == "vless" && "$inner_port" == "$internal_port" && "$inner_listen" == "127.0.0.1" && "$security" == "reality" ]]; then
                            is_reality_pair=true
                            vless_idx=$j
                            dokodemo_tag=$tag
                            processed_vless_indices+=($j) # 标记此vless已被处理
                            break
                        fi
                    fi
                done
            ...
            if [[ "$is_reality_pair" == "true" ]]; then
                active_node_indices+=($i) # 存储dokodemo的索引
                active_node_descriptions+=("REALITY节点 [外部端口:${port}, 内部端口:${internal_port}]")
                active_is_reality+=("true")
                active_reality_vless_index+=($vless_idx)
                active_reality_dokodemo_tag+=("$dokodemo_tag")
                echo -e "${GREEN}[${count}] ${active_node_descriptions[-1]}${PLAIN}"
                count=$((count+1))
            elif [[ ! ("$protocol" == "vless" && "$listen" == "127.0.0.1") ]]; then # 排除REALITY的内部vless
                active_node_indices+=($i)
                active_node_descriptions+=("${protocol}节点 [端口:${port}]")
                active_is_reality+=("false")
                active_reality_vless_index+=(-1)
                active_reality_dokodemo_tag+=("")
                echo -e "${GREEN}[${count}] ${active_node_descriptions[-1]}${PLAIN}"
                count=$((count+1))
            fi
        done

        if [[ ${#active_node_indices[@]} -eq 0 ]]; then
            echo -e "${YELLOW}没有活动的节点可以禁用！${PLAIN}"
            return
        fi

        read -p "请输入要禁用的节点编号 [0-$((${#active_node_indices[@]}-1))]: " disable_choice
        if [[ ! "$disable_choice" =~ ^[0-9]+$ ]] || [ "$disable_choice" -ge "${#active_node_indices[@]}" ]; then
            echo -e "${RED}输入无效！${PLAIN}"
            return
        fi

        target_index=${active_node_indices[$disable_choice]}
        is_reality=${active_is_reality[$disable_choice]}

        if [[ "$is_reality" == "true" ]]; then
            # 禁用 REALITY 节点对
            dokodemo_idx=$target_index
            vless_idx=${active_reality_vless_index[$disable_choice]}
            dokodemo_tag=${active_reality_dokodemo_tag[$disable_choice]}

            echo -e "${YELLOW}正在禁用 REALITY 节点 (dokodemo索引: $dokodemo_idx, vless索引: $vless_idx)...${PLAIN}"

            # 获取要移动的配置 (注意顺序，先获取索引大的)
            if [[ $vless_idx -gt $dokodemo_idx ]]; then
                vless_node_config=$(jq ".inbounds[$vless_idx]" <<< "$temp_config")
                dokodemo_node_config=$(jq ".inbounds[$dokodemo_idx]" <<< "$temp_config")
            else
                dokodemo_node_config=$(jq ".inbounds[$dokodemo_idx]" <<< "$temp_config")
                vless_node_config=$(jq ".inbounds[$vless_idx]" <<< "$temp_config")
            fi

            # 添加到 disabled_inbounds
            temp_config=$(jq --argjson node "$dokodemo_node_config" '.disabled_inbounds += [$node]' <<< "$temp_config")
            temp_config=$(jq --argjson node "$vless_node_config" '.disabled_inbounds += [$node]' <<< "$temp_config")

            # 从 inbounds 删除 (注意索引变化，先删除索引大的)
             if [[ $vless_idx -gt $dokodemo_idx ]]; then
                temp_config=$(jq "del(.inbounds[$vless_idx])" <<< "$temp_config")
                temp_config=$(jq "del(.inbounds[$dokodemo_idx])" <<< "$temp_config")
            else
                temp_config=$(jq "del(.inbounds[$dokodemo_idx])" <<< "$temp_config")
                temp_config=$(jq "del(.inbounds[$vless_idx])" <<< "$temp_config") # vless索引不变，因为dokodemo索引更大
            fi

            # 删除相关路由规则
            echo -e "${YELLOW}正在删除与 $dokodemo_tag 相关的路由规则...${PLAIN}"
            temp_config=$(jq "del(.routing.rules[]? | select(.inboundTag != null and .inboundTag[0] == \"$dokodemo_tag\"))" <<< "$temp_config")

            echo -e "${GREEN}REALITY 节点已禁用！${PLAIN}"

        else
            # 禁用普通节点
            echo -e "${YELLOW}正在禁用 ${active_node_descriptions[$disable_choice]} (索引: $target_index)...${PLAIN}"
            node_config=$(jq ".inbounds[$target_index]" <<< "$temp_config")

            # 添加到 disabled_inbounds
            temp_config=$(jq --argjson node "$node_config" '.disabled_inbounds += [$node]' <<< "$temp_config")
            # 从 inbounds 删除
            temp_config=$(jq "del(.inbounds[$target_index])" <<< "$temp_config")

            echo -e "${GREEN}节点已禁用！${PLAIN}"
        fi

    elif [[ "$action_choice" == "2" ]]; then
        # --- 启用禁用节点 ---
        echo -e "${YELLOW}当前禁用的节点列表:${PLAIN}"
        declare -a disabled_node_indices
        declare -a disabled_node_descriptions
        declare -a disabled_is_reality
        declare -a disabled_reality_pair_index # 存储配对的另一个节点的索引

        disabled_count=$(jq '.disabled_inbounds | length' <<< "$temp_config")
        count=0
        processed_disabled_indices=() # 存储已作为REALITY一部分处理的索引

        for ((i=0; i<${disabled_count}; i++)); do
             # 检查此索引是否已被作为REALITY的一部分处理过
            skip=false
            for processed_idx in "${processed_disabled_indices[@]}"; do
                if [[ "$i" == "$processed_idx" ]]; then
                    skip=true
                    break
                fi
            ...
            protocol=$(jq -r ".disabled_inbounds[$i].protocol" <<< "$temp_config")
            port=$(jq -r ".disabled_inbounds[$i].port" <<< "$temp_config")
            tag=$(jq -r ".disabled_inbounds[$i].tag // \"未命名\"" <<< "$temp_config")
            listen=$(jq -r ".disabled_inbounds[$i].listen // \"0.0.0.0\"" <<< "$temp_config")

            is_reality_pair=false
            pair_idx=-1

            # 检查是否是REALITY的dokodemo-door或vless部分
            if [[ "$protocol" == "dokodemo-door" && ("$tag" == "dokodemo-in" || "$tag" =~ ^dokodemo-in-[0-9]+$) ]]; then
                internal_port=$(jq -r ".disabled_inbounds[$i].settings.port" <<< "$temp_config")
                # 寻找对应的vless
                for ((j=0; j<${disabled_count}; j++)); do
                    if [[ $i -ne $j ]]; then
                         inner_protocol=$(jq -r ".disabled_inbounds[$j].protocol" <<< "$temp_config")
                         inner_port=$(jq -r ".disabled_inbounds[$j].port" <<< "$temp_config")
                         inner_listen=$(jq -r ".disabled_inbounds[$j].listen // \"0.0.0.0\"" <<< "$temp_config")
                         security=$(jq -r ".disabled_inbounds[$j].streamSettings.security // \"none\"" <<< "$temp_config")
                         if [[ "$inner_protocol" == "vless" && "$inner_port" == "$internal_port" && "$inner_listen" == "127.0.0.1" && "$security" == "reality" ]]; then
                             is_reality_pair=true
                             pair_idx=$j
                             processed_disabled_indices+=($j) # 标记配对的vless已被处理
                             break
                         fi
                    fi
                done
                if [[ "$is_reality_pair" == "true" ]]; then
                    disabled_node_indices+=($i) # 存储dokodemo的索引
                    disabled_node_descriptions+=("REALITY节点 [外部端口:${port}, 内部端口:${internal_port}]")
                    disabled_is_reality+=("true")
                    disabled_reality_pair_index+=($pair_idx) # 存储vless的索引
                    echo -e "${GREEN}[${count}] ${disabled_node_descriptions[-1]}${PLAIN}"
                    count=$((count+1))
                fi
            elif [[ "$protocol" == "vless" && "$listen" == "127.0.0.1" ]]; then
                 # 检查是否是REALITY的vless部分，但没有找到配对的dokodemo（理论上不应发生，除非手动修改了disabled_inbounds）
                 security=$(jq -r ".disabled_inbounds[$i].streamSettings.security // \"none\"" <<< "$temp_config")
                 if [[ "$security" == "reality" ]]; then
                     # 这是一个孤立的REALITY vless部分，也列出来但标记为可能异常
                     disabled_node_indices+=($i)
                     disabled_node_descriptions+=("孤立的REALITY VLESS部分 [内部端口:${port}]")
                     disabled_is_reality+=("false") # 标记为非配对处理
                     disabled_reality_pair_index+=(-1)
                     echo -e "${YELLOW}[${count}] ${disabled_node_descriptions[-1]}${PLAIN}"
                     count=$((count+1))
                 else
                    # 其他监听127.0.0.1的vless（非REALITY），正常列出
                    disabled_node_indices+=($i)
                    disabled_node_descriptions+=("${protocol}节点 [端口:${port}, 监听:127.0.0.1]")
                    disabled_is_reality+=("false")
                    disabled_reality_pair_index+=(-1)
                    echo -e "${GREEN}[${count}] ${disabled_node_descriptions[-1]}${PLAIN}"
                    count=$((count+1))
                 fi
                ...
                # 其他普通节点
                disabled_node_indices+=($i)
                disabled_node_descriptions+=("${protocol}节点 [端口:${port}]")
                disabled_is_reality+=("false")
                disabled_reality_pair_index+=(-1)
                echo -e "${GREEN}[${count}] ${disabled_node_descriptions[-1]}${PLAIN}"
                count=$((count+1))
        ...
        if [[ ${#disabled_node_indices[@]} -eq 0 ]]; then
            echo -e "${YELLOW}没有禁用的节点可以启用！${PLAIN}"
            return
        fi

        read -p "请输入要启用的节点编号 [0-$((${#disabled_node_indices[@]}-1))]: " enable_choice
        if [[ ! "$enable_choice" =~ ^[0-9]+$ ]] || [ "$enable_choice" -ge "${#disabled_node_indices[@]}" ]; then
            echo -e "${RED}输入无效！${PLAIN}"
            return
        fi

        target_index=${disabled_node_indices[$enable_choice]}
        is_reality=${disabled_is_reality[$enable_choice]}

        if [[ "$is_reality" == "true" ]]; then
            # 启用 REALITY 节点对
            dokodemo_idx=$target_index
            vless_idx=${disabled_reality_pair_index[$enable_choice]}

            echo -e "${YELLOW}正在启用 REALITY 节点 (dokodemo索引: $dokodemo_idx, vless索引: $vless_idx 在 disabled_inbounds 中)...${PLAIN}"

            # 获取要移动的配置 (注意顺序，先获取索引大的)
            if [[ $vless_idx -gt $dokodemo_idx ]]; then
                vless_node_config=$(jq ".disabled_inbounds[$vless_idx]" <<< "$temp_config")
                dokodemo_node_config=$(jq ".disabled_inbounds[$dokodemo_idx]" <<< "$temp_config")
            else
                dokodemo_node_config=$(jq ".disabled_inbounds[$dokodemo_idx]" <<< "$temp_config")
                vless_node_config=$(jq ".disabled_inbounds[$vless_idx]" <<< "$temp_config")
            fi
            ...
            # 添加回 inbounds
            temp_config=$(jq --argjson node "$dokodemo_node_config" '.inbounds += [$node]' <<< "$temp_config")
            temp_config=$(jq --argjson node "$vless_node_config" '.inbounds += [$node]' <<< "$temp_config")

            # 从 disabled_inbounds 删除 (注意索引变化，先删除索引大的)
            if [[ $vless_idx -gt $dokodemo_idx ]]; then
                temp_config=$(jq "del(.disabled_inbounds[$vless_idx])" <<< "$temp_config")
                temp_config=$(jq "del(.disabled_inbounds[$dokodemo_idx])" <<< "$temp_config")
            else
                temp_config=$(jq "del(.disabled_inbounds[$dokodemo_idx])" <<< "$temp_config")
                temp_config=$(jq "del(.disabled_inbounds[$vless_idx])" <<< "$temp_config")
            fi
            ...
            # 重新添加路由规则
            dokodemo_tag=$(jq -r '.tag' <<< "$dokodemo_node_config")
            server_name=$(jq -r '.streamSettings.realitySettings.serverNames[0]' <<< "$vless_node_config")

            echo -e "${YELLOW}正在重新添加与 $dokodemo_tag 相关的路由规则 (目标SNI: $server_name)...${PLAIN}"

            # 确保 .routing 和 .routing.rules 存在
            if ! jq -e '.routing' <<< "$temp_config" > /dev/null; then
                temp_config=$(jq '. + {"routing": {"rules": []}}' <<< "$temp_config")
            elif ! jq -e '.routing.rules' <<< "$temp_config" > /dev/null; then
                 temp_config=$(jq '.routing += {"rules": []}' <<< "$temp_config")
            fi

            # 创建路由规则
            routing_rule_domain=$(jq -n --arg tag "$dokodemo_tag" --arg sni "$server_name" \
                '{inboundTag: [$tag], domain: [$sni], outboundTag: "direct"}')
            routing_rule_block=$(jq -n --arg tag "$dokodemo_tag" \
                '{inboundTag: [$tag], outboundTag: "block"}')

            # 添加规则到 routing.rules 数组末尾
            temp_config=$(jq --argjson rule1 "$routing_rule_domain" --argjson rule2 "$routing_rule_block" \
                '.routing.rules += [$rule1, $rule2]' <<< "$temp_config")

            echo -e "${GREEN}REALITY 节点已启用！${PLAIN}"

        else
            # 启用普通节点
            echo -e "${YELLOW}正在启用 ${disabled_node_descriptions[$enable_choice]} (索引: $target_index 在 disabled_inbounds 中)...${PLAIN}"
            node_config=$(jq ".disabled_inbounds[$target_index]" <<< "$temp_config")

            # 添加回 inbounds
            temp_config=$(jq --argjson node "$node_config" '.inbounds += [$node]' <<< "$temp_config")
            # 从 disabled_inbounds 删除
            temp_config=$(jq "del(.disabled_inbounds[$target_index])" <<< "$temp_config")

            echo -e "${GREEN}节点已启用！${PLAIN}"
        fi
    ...
    if jq empty ${CONFIG_FILE} > /dev/null 2>&1; then
        echo -e "${GREEN}配置文件更新成功，正在重启 Xray...${PLAIN}"
        systemctl restart xray
        if systemctl is-active --quiet xray; then
            echo -e "${GREEN}Xray 服务已成功重启！${PLAIN}"
        else
            echo -e "${RED}Xray 服务重启失败！请检查配置。正在尝试恢复备份...${PLAIN}"
            cp ${CONFIG_FILE}.bak_toggle_$(date +%Y%m%d%H%M%S) ${CONFIG_FILE}
            systemctl restart xray
            echo -e "${YELLOW}已恢复备份文件并重启服务。${PLAIN}"
        fi
    else
        echo -e "${RED}错误：更新后的配置文件 JSON 格式无效！正在恢复备份...${PLAIN}"
        cp ${CONFIG_FILE}.bak_toggle_$(date +%Y%m%d%H%M%S) ${CONFIG_FILE}
        echo -e "${YELLOW}已恢复备份文件。请检查脚本或手动编辑配置文件。${PLAIN}"
    fi
}
