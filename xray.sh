...
view_stats() {
    ...
    if [[ -f ${CONFIG_FILE} ]]; then
        ...
        for ((i=0; i<${inbound_count}; i++)); do
            ...
            if [[ -n "$tag" ]]; then
                # 查询入站流量
                uplink=$(xray api stats --server=127.0.0.1:62789 --name "inbound>>>${tag}>>>traffic>>>uplink" | jq -r '.stat.value')
                downlink=$(xray api stats --server=127.0.0.1:62789 --name "inbound>>>${tag}>>>traffic>>>downlink" | jq -r '.stat.value')
